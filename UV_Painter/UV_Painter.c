...
#define N_LED 8
#define MAGIC_NUMBER 10 //number of times to illuminate a column before moving onto the next
#define N_ELEMENTS 97//number of elements/columns in the message
...
const uint8_t mFangletronicsChars[97][N_LED] PROGMEM = {
...
int main()
{
	...
	for(;;++i)
	{
		...
		{
			for(k = 0; k < MAGIC_NUMBER; k++)// TODO remove this completely? will make text more legible
			{
				DDRB = ledsAllOff.mDdrB;
				PORTB = ledsAllOff.mPortB;
				for(l = 0; l < N_LED; l++)
				{
					if(pgm_read_byte(&(mFangletronicsChars[j][l])) > i)
					{
